...
def pending_jobs():
    pending_jobs = db.session.scalars(sa.select(Job).where(Job.assigned_to_id == current_user.id, Job.status.in_(['incomplete', '']), Job.date_scheduled <= datetime.now(timezone=utc))).all()
