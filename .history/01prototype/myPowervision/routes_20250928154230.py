...
def assign_permissions(role_id):
    ...
    assigned = db.session.scalars(sa.select(role_permission.c.permission_id).where(role_permission.c.role_id == role_id)).all()
    if not role:
        flash('Role not found!', 'danger')
        return redirect(url_for('roleManagement'))
    if not assigned:
        flash('No permissions assigned!', 'success')

    return render_template('user_management/assign_permissions.html', title='Assign Permissions', active='assign permissions', role=role, permissions=permissions, assigned=assigned)

@app.route("/assign-permission/<int:role_id>/<int:permission_id>", methods=['GET', 'POST'])
def assign_permission(role_id, permission_id):
    role = db.session.scalar(sa.select(Role).where(Role.id == role_id))
    permission = db.session.scalar(sa.select(Permission).where(Permission.id == permission_id))
    assigned = db.session.scalars(sa.select(role_permission.c.permission_id).where(role_permission.c.role_id == role_id)).all()

    perm = role_permission(role_permission.c.role_id == role_id).where(role_permission.c.permission_id == permission_id)
    db.session.add(perm)
    db.session.commit()
    
    flash('Permission assigned!', 'success')
...
def unassign_permission(role_id, permission_id):
    ...
    perm = role_permission.delete().where(role_permission.c.role_id == role_id).where(role_permission.c.permission_id == permission_id)
    db.session.execute(perm)
    db.session.commit()
    flash('Permission unassigned!', 'success')
    return redirect(url_for('assign_permissions', role_id=role_id))
# ---------------------------------Permissions--------------------------------------
