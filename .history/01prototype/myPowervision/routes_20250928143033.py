...
@app.route("/assign-permissions/role_id")
def assign_permission(role_id):
    role = db.session.scalar(sa.select(Role).where(Role.id==role_id))
    return render_template('user_management/assign_permissions.html', title='Assign Permissions', active='assign permissions',role=role)
...
def edit_role(role_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('roleManagement'))
    return render_template('user_management/edit_role.html', title='Edit Role', active='roles', form=form, role=role)
...
def delete_role(role_id):
    ...
    if role:
        ...
        flash('Role not found!', category='danger')
    return redirect(url_for('roleManagement'))
