...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        user = db.session.execute(sa.select(User).filter_by(username=username.data)).scalar_one_or_none()
        if user is not None:
            raise ValueError('Please use a different username.')

    def validate_email(self, email):
        user = db.session.execute(sa.select(User).filter_by(email=email.data)).scalar_one_or_none()
        if user is not None:
