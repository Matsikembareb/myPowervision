...
def all_scheduled_jobs():
    ...
    return render_template('job_management/technician/scheduled.html', title='My Jobs', active='jobs', jobs=all_scheduled_jobs)
...
def all_pending_jobs():
    ...
    return render_template('job_management/technician/pending.html', title='My Jobs', active='jobs', jobs=all_pending_jobs)

@app.route("/all-completed-jobs", methods=['GET','POST'])
def all_completed_jobs():
    all_completed_jobs = db.session.scalars(sa.select(Job).where(Job.status.in_(['completed', 'closed']))).all()
    return render_template('job_management/technician/completed.html', title='My Jobs', active='jobs', jobs=all_completed_jobs)

@app.route("/all-closed-jobs", methods=['GET','POST'])
def all_closed_jobs():
    all_closed_jobs = db.session.scalars(sa.select(Job).where(Job.status == 'closed')).all()
    return render_template('job_management/technician/closed.html', title='My Jobs', active='jobs', jobs=all_closed_jobs)
