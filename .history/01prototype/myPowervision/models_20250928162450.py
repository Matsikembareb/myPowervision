...
class User(UserMixin, db.Model):
    ...
    jobs: so.WriteOnlyMapped['Job'] = so.relationship(back_populates='user_account')
    created_jobs: so.WriteOnlyMapped['Job'] = so.relationship(back_populates='created_by')
    assigned_jobs: so.WriteOnlyMapped['Job'] = so.relationship(back_populates='assigned_to')
...
class Customer(db.Model):
    ...
    relationship_status: so.Mapped[Optional[str]] = so.mapped_column(sa.String(64), default='active')
...
class Job(db.Model):
    ...
    assigned_to_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('staff.id'), index=True)
    created_by_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('staff.id'), index=True)
    # Relationships
    customer: so.Mapped[Optional['Customer']] = so.relationship(back_populates='jobs')
    assigned_to: so.Mapped[Optional['Staff']] = so.relationship(back_populates='assigned_jobs')
    created_by: so.Mapped[Optional['Staff']] = so.relationship(back_populates='created_jobs')
