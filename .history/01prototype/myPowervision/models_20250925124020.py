...
class User(db.Model):
    ...
    role: so.Mapped[Optional['Role']] = so.relationship(back_populates='delegates')
    # ADD THIS: Foreign key to User
    user_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('user.id'), unique=True, index=True, default='external')
    # One-to-one relationship (User has one Staff record)
