...
def permissionManagement():
    ...
    roles = db.session.scalars(sa.select(Role)).all()
    role_permission = db.session.scalars(sa.select(role_permission)).all()
    return render_template('user_management/permissions.html', title='Permission Management', active='permissions', permissions=permissions, roles=roles, role_permission=role_permission)
