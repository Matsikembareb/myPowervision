...
def assign_permission(role_id):
    role = db.session.scalars(sa.select(Role)).all()
    return render_template('user_management/assign_permissions.html', title='Assign Permissions', active='assign permissions',roles=)
