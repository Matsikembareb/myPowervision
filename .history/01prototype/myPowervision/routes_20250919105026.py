...
def userDashboard():
    ...
    if session.get('user_id'):
        ...
        user = User.query.filter_by(id=id).first()  # Remove ()
    ...
    if request.method == 'POST':
        ...
        if email == '' or password == '':
            ...
            if user:
                ...
                User.query.filter_by(email=email).first().update(dict(password=hashed_password))
                db.session.commit()
...
def userUpdateProfile():
    if not session.get('user_id'):
        return redirect(url_for('userIndex'))
    if session.get('user_id'):
        user = User.query.filter_by(id=session.get('user_id')).first()  # Remove ()
        if request.method == 'POST':
            # process the form data
            fname = request.form.get('fname')
            lname = request.form.get('lname')
            ...
            if fname == '' or lname == '' or email == '' or username == '' or edu == '':
                ...
                if is_email and is_email.id != user.id:
                    ...
                    User.query.filter_by(id=user.id).first().update(dict(fname=fname, lname=lname, email=email, username=username, edu=edu))
                    db.session.commit()
