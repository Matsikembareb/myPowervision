...
from flask import render_template, url_for, redirect, flash, request, session  # Add session
...
def edit_role(role_id):
    ...
    return render_template('user_management/edit_role.html', title='Edit Role', form=form, role=role)
...
def assign_permissions(role_id):  # Changed function name to plural to match template
    role = db.session.scalar(sa.select(Role).where(Role.id == role_id))
    ...
    if request.method == 'POST':
        # Handle permission assignment here
    ...
    return render_template('user_management/assign_permissions.html', 
                         title='Assign Permissions', active='assign permissions', 
