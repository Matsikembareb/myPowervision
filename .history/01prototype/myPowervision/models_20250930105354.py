...
class User(UserMixin, db.Model):
    ...
    def is_developer(self):
        ...
        if not self.staff:
            return False
        return (self.staff.department and 
                self.staff.department.name == 'IT Department')
    
    def is_executive(self):
        """Check if user is in executive/management role"""
        if not self.staff:
            return False
        # Check if user has executive position or department
        executive_positions = ['Manager', 'Director', 'Executive', 'CEO', 'CTO', 'CFO']
        if self.staff.position:
            return any(exec_pos.lower() in self.staff.position.title.lower() 
                      for exec_pos in executive_positions)
        return False
    ...
    def is_logistics(self):
        ...
        if not self.staff:
            return False
        return (self.staff.department and 
                self.staff.department.name.lower() in ['logistics', 'operations', 'warehouse'])
    ...
    def is_stores(self):
        ...
        if not self.staff:
            return False
        return (self.staff.department and 
                'store' in self.staff.department.name.lower())
    
    def is_technical(self):
        """Check if user is in technical role"""
        if not self.staff:
            return False
        technical_departments = ['IT Department', 'Technical', 'Engineering']
        return (self.staff.department and 
                self.staff.department.name in technical_departments)
