...
class StaffForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.department.choices = [(dept.id, dept.title) for dept in Department.query.all()]
        self.position.choices = [(pos.id, pos.title) for pos in Position.query.all()]

class DepartmentForm(FlaskForm):
    title = StringField('Department Title', validators=[DataRequired(), Length(min=1, max=64)])
    description = TextAreaField('Description', validators=[Length(max=256)])
    submit = SubmitField('Submit')

    def validate_title(self, title):
        department = db.session.scalar(sa.select(Department).where(Department.title == title.data))
        if department is not None:
            raise ValidationError('Department already exists. Please use a different title.')
