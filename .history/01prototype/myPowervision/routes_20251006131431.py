...
from myPowervision.forms import DepartmentForm, LoginForm, PositionForm, RegistrationForm, RoleForm, PermissionForm, StaffForm, JobForm, CustomerForm, VehicleForm
from myPowervision.models import Department, Permission, Role, Staff, User, Vehicle, role_permission, Job, Position, Customer, job_assistant
...
def create_job():
    ...
    if form.validate_on_submit():
        ...
        total_minutes = (form.time_allocated_hours.data or 0) * 60 + (form.time_allocated_minutes.data or 0)
        ...
        new_job = Job(
            ...
            vehicle_id=form.vehicle.data if form.vehicle.data != 0 else None,
            date_scheduled=form.date_scheduled.data,
            time_allocated=total_minutes if total_minutes > 0 else None,  # Store as minutes
        ...
        # Add assistants to the job
        if form.assistants.data:
            for assistant_id in form.assistants.data:
                if assistant_id != 0:  # Skip any invalid selections
                    assistant = db.session.get(User, assistant_id)
                    if assistant:
                        new_job.assistants.append(assistant)
...
def create_vehicle():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Vehicle created successfully!', category='success')
        return redirect(url_for('vehicleManagement'))
    return render_template('vehicle_management/create_vehicle.html', title='Create Vehicle', active='vehicles', form=form)

@app.route("/vehicles")
def vehicleManagement():
    vehicles = db.session.scalars(sa.select(Vehicle)).all()
    return render_template('vehicle_management/vehicles.html', title='Vehicle Management', active='vehicles', vehicles=vehicles)
...
def technician_jobs():
    # Get jobs where user is directly assigned OR is an assistant
    assigned_jobs = db.session.scalars(sa.select(Job).where(Job.assigned_to_id == current_user.id)).all()
    assistant_jobs = db.session.scalars(
        sa.select(Job).join(job_assistant).where(job_assistant.c.user_id == current_user.id)
    ).all()
    
    # Combine both lists and remove duplicates
    all_jobs = list(set(assigned_jobs + assistant_jobs))
    
    return render_template('job_management/technician/assigned_jobs.html', title='My Jobs', active='jobs', jobs=all_jobs)
...
def technician_update_job(job_id):
    ...
    # Check if user is assigned to this job OR is an assistant
    is_assigned = (job.assigned_to_id == current_user.id)
    is_assistant = (current_user in job.assistants)
    
    if not (is_assigned or is_assistant):
        flash('You are not authorized to update this job!', category='danger')
        return redirect(url_for('technician_jobs'))
    ...
    return render_template('job_management/technician/update_job.html', 
                         title='Update Job', 
                         active='jobs', 
                         job=job,
...
def complete_job(job_id):
    ...
    if not (is_assigned or is_assistant):
        flash('You are not authorized to complete this job!', category='danger')
        return redirect(url_for('technician_jobs'))
...
def scheduled_jobs():
    # Jobs where user is assigned OR assistant AND scheduled for future
    assigned_scheduled = db.session.scalars(
        sa.select(Job).where(
            Job.assigned_to_id == current_user.id, 
            Job.date_scheduled > datetime.now(timezone.utc)
        )
    ).all()
    
    assistant_scheduled = db.session.scalars(
        sa.select(Job).join(job_assistant).where(
            job_assistant.c.user_id == current_user.id,
            Job.date_scheduled > datetime.now(timezone.utc)
        )
    ).all()
    
    scheduled_jobs = list(set(assigned_scheduled + assistant_scheduled))
    return render_template('job_management/technician/scheduled.html', title='My Scheduled Jobs', active='jobs', jobs=scheduled_jobs)

@app.route("/pending-jobs", methods=['GET','POST'])
def pending_jobs():
    # Jobs where user is assigned OR assistant AND pending
    assigned_pending = db.session.scalars(
        sa.select(Job).where(
            Job.assigned_to_id == current_user.id, 
            Job.status.in_(['incomplete', '']), 
            Job.date_scheduled <= datetime.now()
        )
    ).all()
    
    assistant_pending = db.session.scalars(
        sa.select(Job).join(job_assistant).where(
            job_assistant.c.user_id == current_user.id,
            Job.status.in_(['incomplete', '']),
            Job.date_scheduled <= datetime.now()
    ...
    pending_jobs = list(set(assigned_pending + assistant_pending))
    return render_template('job_management/technician/pending.html', title='My Pending Jobs', active='jobs', jobs=pending_jobs)

@app.route("/completed-jobs", methods=['GET','POST'])
def completed_jobs():
    # Jobs where user is assigned OR assistant AND completed
    assigned_completed = db.session.scalars(
        sa.select(Job).where(
            Job.assigned_to_id == current_user.id, 
    ...
    assistant_completed = db.session.scalars(
        sa.select(Job).join(job_assistant).where(
            job_assistant.c.user_id == current_user.id,
            Job.status.in_(['completed', 'closed'])
        )
    ...
    completed_jobs = list(set(assigned_completed + assistant_completed))
    return render_template('job_management/technician/completed.html', title='My Completed Jobs', active='jobs', jobs=completed_jobs)
