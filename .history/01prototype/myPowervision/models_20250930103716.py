...
class User(UserMixin, db.Model):
    ...
    # Fix job relationships with explicit foreign_keys
    created_jobs: so.WriteOnlyMapped['Job'] = so.relationship(
        foreign_keys='Job.created_by_id',  # Specify which FK in Job table to use
        back_populates='created_by'
    )
    assigned_jobs: so.WriteOnlyMapped['Job'] = so.relationship(
        foreign_keys='Job.assigned_to_id',  # Specify which FK in Job table to use
    ...
    def is_developer(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'IT Department' else False
    def is_executive(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'Executive Department' else False
    def is_logistics(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'Logistics Department' else False
    def is_accounts(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'Accounts Department' else False
    def is_technical(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'Technical Department' else False
    def is_stores(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.department.name == 'Stores Department' else False
    def is_technical_manager(self):
        staff = db.session.query(Staff).filter_by(user_id=self.id).first()
        return True if staff and staff.position and staff.position.title == 'Technical Manager' else False
#Roles class
class Role(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
...
class Job(db.Model):
    ...
    job_number: so.Mapped[str] = so.mapped_column(sa.String(20), unique=True, index=True)
    stores_confirmation: so.Mapped[Optional[bool]] = so.mapped_column(sa.Boolean, default=False)
    work_done: so.Mapped[Optional[bool]] = so.mapped_column(sa.Boolean, default=False)
    technical_manager_approval: so.Mapped[Optional[bool]] = so.mapped_column(sa.Boolean, default=False)
    accounts_payment: so.Mapped[Optional[bool]] = so.mapped_column(sa.Boolean, default=False)
