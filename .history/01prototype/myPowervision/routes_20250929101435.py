...
from myPowervision.forms import LoginForm, RegistrationForm, RoleForm, PermissionForm, StaffForm, JobForm
from myPowervision.models import Permission, Role, Staff, User, role_permission, Job, Position, Customer
...
def create_position():
    form = StaffForm()
    if form.validate_on_submit():
        new_position = Position(title=form.position.data, description=form.department.data)
        db.session.add(new_position)
        db.session.commit()
        flash('Position created successfully!', category='success')
        return redirect(url_for('positionManagement'))
    return render_template('staff_management/create_position.html', title='Create Position', active='positions', form=form)
