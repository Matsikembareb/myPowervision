...
class User(UserMixin, db.Model):
    ...
    # Contact person for jobs
    contact_jobs: so.WriteOnlyMapped['Job'] = so.relationship(
        foreign_keys='Job.contact_person_id',
        back_populates='contact_person'
    )
    
    # Many-to-many relationship for job assistants
    assistant_jobs: so.Mapped[list['Job']] = so.relationship(
        secondary=job_assistants,
...
class Job(db.Model):
    ...
    contact_person_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('user.id'), index=True)  # NEW
    vehicle_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('vehicle.id'), index=True)  # NEW
    ...
    # NEW: Contact person relationship
    contact_person: so.Mapped[Optional['User']] = so.relationship(
        foreign_keys=[contact_person_id],
        back_populates='contact_jobs'
    )
    
    # NEW: Vehicle relationship
    vehicle: so.Mapped[Optional['Vehicle']] = so.relationship(back_populates='jobs')
    
    # NEW: Many-to-many relationship with assistants
