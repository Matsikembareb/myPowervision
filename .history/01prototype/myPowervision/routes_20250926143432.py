...
from myPowervision.forms import LoginForm, RegistrationForm, RoleForm
...
@app.route("/roles")
def roleManagement():
    return render_template('user_management/roles.html', title='Role Management', active='roles')
...
def create_role():
    ...
    if form.validate_on_submit():
        form.validate_role()
        db.session.add(form.create_role())
        db.session.commit()
        flash('Role created successfully!', category='success')
        return redirect(url_for('roleManagement'))
    return render_template('user_management/create_role.html', title='Create Role', active='roles')

# ---------------------------------Permissions--------------------------------------
@app.route("/permissions")
def permissionManagement():
    return render_template('user_management/permissions.html', title='Permission Management', active='permissions')
    ...
    if request.method == 'POST':
        ...
        if username == '' or password == '':
            ...
            if admin:
                ...
                return redirect(url_for('adminChangePassword'))
    return render_template("auth/admin/change_password.html", title="Change Password", admin=admin)
