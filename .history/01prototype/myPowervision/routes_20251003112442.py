...
def statistics():
    ...
    status_labels = ['Scheduled', 'Pending', 'Completed', 'Closed']
    status_data = [
        db.session.scalar(sa.select(func.count(Job.id)).where(Job.status == 'scheduled')) or 0,
        db.session.scalar(sa.select(func.count(Job.id)).where(Job.status == 'pending')) or 0,
        db.session.scalar(sa.select(func.count(Job.id)).where(Job.status == 'completed')) or 0,
        db.session.scalar(sa.select(func.count(Job.id)).where(Job.status == 'closed')) or 0
    ]
