...
from myPowervision.models import Permission, Role, Staff, User, role_permission, Job
...
def create_job():
    ...
    if form.validate_on_submit():
        new_job = Job(
            ...
            created_by=current_user.username,  # Should be created_by_id (integer)
            assigned_to=form.assigned_to.data, # Should be assigned_to_id (integer)
            customer=form.customer.data        # Should be customer_id (integer)
        )
