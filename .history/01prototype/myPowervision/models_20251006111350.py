...
# Association table for many-to-many relationship between Job and Assistant Users
job_assistant = sa.Table(
    'job_assistant',
    db.metadata,
...
class User(UserMixin, db.Model):
    ...
    assistant_jobs: so.Mapped[list['Job']] = so.relationship(
        secondary=job_assistant,
    ...
    def get_assistant_jobs_count(self):
        """Get count of jobs where user is an assistant"""
        return len(self.assistant_jobs) if self.assistant_jobs else 0
...
#Jobs class - UPDATED with assistants and contact person
class Job(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    ...
    contact_person: so.Mapped[Optional[str]] = so.mapped_column(sa.String(128))  # Contact person for this specific job
    contact_phone: so.Mapped[Optional[str]] = so.mapped_column(sa.String(20))
    contact_email: so.Mapped[Optional[str]] = so.mapped_column(sa.String(128))
    ...
    vehicle: so.Mapped[Optional['Vehicle']] = so.relationship(back_populates='jobs')
    
    # User relationships with explicit foreign_keys
    ...
    # Many-to-many relationship with Users (assistants)
    assistants: so.Mapped[list['User']] = so.relationship(
        secondary=job_assistant,
        back_populates='assistant_jobs'
    )
    ...
    def get_assistants_names(self):
        """Get comma-separated list of assistant names"""
        if not self.assistants:
            return "None"
        return ", ".join([assistant.username for assistant in self.assistants])
    
    def add_assistant(self, user):
        """Add an assistant to the job"""
        if user not in self.assistants:
            self.assistants.append(user)
    
    def remove_assistant(self, user):
        """Remove an assistant from the job"""
        if user in self.assistants:
