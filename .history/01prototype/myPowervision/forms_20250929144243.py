...
class StaffForm(FlaskForm):
    ...
    # FIX: Remove Length validator from hire_date since it's a datetime field
    hire_date = DateField('Hire Date', validators=[Optional()])  # Use DateField instead
    ...
    def validate_email(self, email):
        ...
        if self._obj is not None:
            staff = db.session.scalar(sa.select(Staff).where(Staff.email == email.data, Staff.id != self._obj.id))
        else:
