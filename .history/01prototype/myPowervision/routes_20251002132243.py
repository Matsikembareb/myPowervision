...
def scheduled_jobs():
    scheduled_jobs = db.session.scalars(sa.select(Job).where(Job.assigned_to_id == current_user.id, Job.date_scheduled > datetime.now())).all()
...
def pending_jobs():
    pending_jobs = db.session.scalars(sa.select(Job).where(Job.assigned_to_id == current_user.id, Job.status == ('incomplete' or ''), Job.date_scheduled <= datetime.now())).all()
...
def completed_jobs():
    completed_jobs = db.session.scalars(sa.select(Job).where(Job.assigned_to_id == current_user.id, Job.date_completed != None)).all()
