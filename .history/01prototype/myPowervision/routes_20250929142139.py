...
def edit_staff(staff_id):
    staff = db.session.scalar(sa.select(Staff).where(Staff.id == staff_id))
    form = StaffForm(obj=staff)
    if form.validate_on_submit():
        staff.staff_number = form.staff_number.data
        staff.name = form.name.data
        staff.surname = form.surname.data
        staff.email = form.email.data
        staff.phone = form.phone.data
        staff.department_id = form.department.data
        staff.position_id = form.position.data
        db.session.commit()
        flash('Staff updated successfully!', category='success')
        return redirect(url_for('staffManagement'))
    return render_template('staff_management/edit_staff.html', title='Edit Staff', active='staff', form=form)
...
def create_customer():
    ...
    return render_template('customer_management/create_customer.html', title='Create Customer', active='customers')
