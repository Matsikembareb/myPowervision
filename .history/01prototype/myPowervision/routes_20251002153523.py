...
def techUnverified():
    jobs = db.session.scalars(sa.select(Job).where(Job.work_done != True)).all()
    return render_template('job_management/helpdesk/unverified_jobs.html', title='tech Unverified Jobs', active='jobs', jobs=jobs)

@app.route("/tech-verified")
def techVerified():
    jobs = db.session.scalars(sa.select(Job).where(Job.work_done == True)).all()
    return render_template('job_management/helpdesk/verified_jobs.html', title='tech Verified Jobs', active='jobs', jobs=jobs)
...
def tech_verify_job(job_id):
    ...
    if request.method == 'POST':
        ...
        if job_number_verify == "":
            flash('Please enter a job number to verify.', category='danger')
            return redirect(url_for('tech_verify_job', job_id=job.id))
        job.work_done_by_id = current_user.id
        job.job_number = job_number_verify
        job.work_done = True
        db.session.commit()
