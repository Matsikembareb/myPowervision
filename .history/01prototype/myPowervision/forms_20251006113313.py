...
# Define TimeField outside of the JobForm class
class TimeWidget(object):
    """
    Widget for time input using separate hour and minute fields
    """
    def __call__(self, field, **kwargs):
        kwargs.setdefault('id', field.id)
        hours_id = kwargs['id'] + '_hours'
        minutes_id = kwargs['id'] + '_minutes'
        
        html = ['<div class="time-input-group">']
        html.append(f'<input type="number" id="{hours_id}" name="{field.name}_hours" min="0" max="23" placeholder="HH" style="width: 60px; margin-right: 5px;" value="{field.hours if hasattr(field, "hours") and field.hours else ""}">')
        html.append('<span style="margin: 0 5px;">:</span>')
        html.append(f'<input type="number" id="{minutes_id}" name="{field.name}_minutes" min="0" max="59" placeholder="MM" style="width: 60px;" value="{field.minutes if hasattr(field, "minutes") and field.minutes else ""}">')
        html.append('</div>')
        return ''.join(html)

class TimeField(StringField):
    """
    Custom field for time input with hours and minutes
    """
    widget = TimeWidget()
    ...
    def process_formdata(self, valuelist):
        if valuelist:
            # Handle both formats: "HH:MM" string or separate hour/minute values
            if len(valuelist) == 1 and ':' in str(valuelist[0]):
                try:
                    hours, minutes = str(valuelist[0]).split(':')
                    self.hours = int(hours) if hours else 0
                    self.minutes = int(minutes) if minutes else 0
                    self.data = f"{self.hours:02d}:{self.minutes:02d}"
                except:
                    self.data = None
            else:
                self.data = None
    ...
    def populate_obj(self, obj, name):
        ...
        if self.hours is not None and self.minutes is not None:
            setattr(obj, name, f"{self.hours:02d}:{self.minutes:02d}")
        else:
...
class JobForm(FlaskForm):
    ...
    # Use IntegerField for time allocation in minutes
    time_allocated = IntegerField('Time Allocated (minutes)', validators=[Optional()], widget=NumberInput(min=0))
    ...
    # Change this from BooleanField to SelectMultipleField
    assistants = SelectMultipleField('Assistants', coerce=int, validators=[Optional()])
