...
class StaffForm(FlaskForm):
    ...
    def validate_email(self, email):
        # Get the original staff object if this is an edit operation
        original_staff = getattr(self, 'original_staff', None)
        
        if original_staff is not None:
            # When editing, exclude current staff member from email check
            staff = db.session.scalar(sa.select(Staff).where(
                sa.and_(Staff.email == email.data, Staff.id != original_staff.id)
            ))
        else:
            # When creating new staff, check if email already exists
    ...
    def validate_staff_number(self, staff_number):
        ...
        if original_staff is not None:
            # When editing, exclude current staff member from staff number check
            staff = db.session.scalar(sa.select(Staff).where(
                sa.and_(Staff.staff_number == staff_number.data, Staff.id != original_staff.id)
            ))
        else:
            # When creating new staff, check if staff number already exists
