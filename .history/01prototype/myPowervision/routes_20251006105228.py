...
def create_vehicle():
    ...
    return render_template('helpdesk/create_vehicle.html', title='Create Vehicle', active='vehicles', form=form)

@app.route("/vehicles")
def vehicleManagement():
    vehicles = db.session.scalars(sa.select(Vehicle)).all()
    return render_template('general/vehicles.html', title='Vehicle Management', active='vehicles', vehicles=vehicles)
