...
def create_department():
    form = DepartmentForm()
    if form.validate_on_submit():
        new_department = Department(department=form.title.data, description=form.description.data)
        db.session.add(new_department)
        db.session.commit()
        flash('Department created successfully!', category='success')
        return redirect(url_for('departmentManagement'))
    return render_template('staff_management/create_department.html', title='Create Department', active='departments', form=form)
# ---------------------------------Positions--------------------------------------
@app.route("/positions")
