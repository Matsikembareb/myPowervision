...
def storesUnverified():
    jobs = db.session.scalars(sa.select(Job).where(Job.work_done != True)).all()
    return render_template('job_management/helpdesk/unverified_jobs.html', title='Stores Unverified Jobs', active='jobs', jobs=jobs)

@app.route("/stores-verified")
def storesVerified():
    jobs = db.session.scalars(sa.select(Job).where(Job.work_done == True)).all()
    return render_template('job_management/helpdesk/verified_jobs.html', title='Stores Verified Jobs', active='jobs', jobs=jobs)
