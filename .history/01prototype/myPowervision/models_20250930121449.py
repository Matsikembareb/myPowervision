...
class User(UserMixin, db.Model):
    ...
    def can_confirm_stores(self):
        """Check if user can confirm stores requirements"""
        return self.is_stores() or self.is_logistics()
    
    def can_mark_work_done(self):
        """Check if user can mark work as done"""
        return self.is_technical() or self.is_developer()
    
    def can_approve_technical(self):
        """Check if user can give technical approval"""
        return self.is_technical_manager()
    
    def can_process_payment(self):
        """Check if user can process payments"""
        return self.is_accounts()
