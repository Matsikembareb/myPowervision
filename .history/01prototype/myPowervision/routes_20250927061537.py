...
def permissionManagement():
    ...
    roles = db.session.scalars(sa.select(role_permission)).all()
    return render_template('user_management/permissions.html', title='Permission Management', active='permissions', permissions=permissions, roles=roles)
