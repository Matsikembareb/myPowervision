...
class StaffForm(FlaskForm):
    ...
    def validate_staff_number(self, staff_number):
        ...
        if hasattr(self, '_obj') and self._obj is not None:
            staff = db.session.scalar(sa.select(Staff).where(
                sa.and_(Staff.staff_number == staff_number.data, Staff.id != self._obj.id)
            ))
        else:
...
class DepartmentForm(FlaskForm):
    ...
    def validate_name(self, name):
        # Fix: Check if editing existing department
        if hasattr(self, '_obj') and self._obj is not None:
            department = db.session.scalar(sa.select(Department).where(
                sa.and_(Department.name == name.data, Department.id != self._obj.id)
            ))
        else:
...
class PositionForm(FlaskForm):
    ...
    def validate_title(self, title):
        # Fix: Check if editing existing position
        if hasattr(self, '_obj') and self._obj is not None:
            position = db.session.scalar(sa.select(Position).where(
                sa.and_(Position.title == title.data, Position.id != self._obj.id)
            ))
        else:
...
class RoleForm(FlaskForm):
    ...
    def validate_name(self, name):
        # Fix: Check if editing existing role
        if hasattr(self, '_obj') and self._obj is not None:
            role = db.session.scalar(sa.select(Role).where(
                sa.and_(Role.name == name.data, Role.id != self._obj.id)
            ))
        else:
...
class PermissionForm(FlaskForm):
    ...
    def validate_permission(self, permission):
        # Fix: Check if editing existing permission
        if hasattr(self, '_obj') and self._obj is not None:
            existing_permission = db.session.scalar(sa.select(Permission).where(
                sa.and_(Permission.permission == permission.data, Permission.id != self._obj.id)
            ))
        else:
...
class JobForm(FlaskForm):
    job_number = StringField('Job Number', validators=[Length(min=1, max=20)])
    invoice_number = StringField('Invoice Number', validators=[Length(min=1, max=20)])
    created_at = DateField('Created Date', validators=[Optional()])
    date_scheduled = DateField('Due Date', validators=[Optional()])
    ...
    payment_terms = StringField('Payment Terms', validators=[Length(max=128)])
...
class CustomerForm(FlaskForm):
    ...
    def validate_email(self, email):
        # Fix: Check if editing existing customer
        if hasattr(self, '_obj') and self._obj is not None:
            customer = db.session.scalar(sa.select(Customer).where(
                sa.and_(Customer.email == email.data, Customer.id != self._obj.id)
            ))
        else:
    ...
    def validate_customer_number(self, customer_number):
        ...
        if hasattr(self, '_obj') and self._obj is not None:
            customer = db.session.scalar(sa.select(Customer).where(
                sa.and_(Customer.customer_number == customer_number.data, Customer.id != self._obj.id)
            ))
        else:
            customer = db.session.scalar(sa.select(Customer).where(Customer.customer_number == customer_number.data))
        
        if customer is not None:
            raise ValidationError('Customer number already exists. Please use a different number.')