...
def scheduled_jobs():
    scheduled_jobs = db.session.scalars(sa.select(Job).where(Job.date_scheduled > datetime.now(timezone.utc))).all()
...
def pending_jobs():
    pending_jobs = db.session.scalars(sa.select(Job).where(Job.status.in_(['incomplete', '']), Job.date_scheduled <= datetime.now())).all()
...
def completed_jobs():
    completed_jobs = db.session.scalars(sa.select(Job).where(Job.status.in_(['completed', 'closed']))).all()
...
def closed_jobs():
    closed_jobs = db.session.scalars(sa.select(Job).where(Job.status == 'closed')).all()
    return render_template('job_management/technician/closed.html', title='My Jobs', active='jobs', jobs=closed_jobs)
