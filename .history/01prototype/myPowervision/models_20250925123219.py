...
class User(db.Model):
    ...
    role_id: so.Mapped[Optional[str]] = so.mapped_column(sa.ForeignKey('role.id'), index=True)
    role: so.Mapped['Role'] = so.relationship(back_populates='delegates')
    staff_id: so.Mapped[Optional[str]] = so.mapped_column(sa.ForeignKey('staff.id'), index=True)
    staff: so.Mapped['Staff'] = so.relationship(back_populates='user_account')
...
class Role(db.Model):
    ...
    delegates:so.WriteOnlyMapped[User] = so.relationship(back_populates='role')
...
class Staff(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    staff_id: so.Mapped[int] = so.mapped_column(sa.String('staff.id'), index=True, unique=True, default='external')
    ...
    position_id: so.Mapped[Optional[str]] = so.mapped_column(sa.ForeignKey('position.id'), index=True)
    department_id: so.Mapped[Optional[str]] = so.mapped_column(sa.ForeignKey('department.id'), index=True)
    ...
    user_account: so.WriteOnlyMapped[Optional[User]] = so.relationship(back_populates='staff', uselist=False)
